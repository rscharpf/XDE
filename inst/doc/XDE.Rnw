...
empirical <- empiricalStart(eList, phenotypeLabel="adenoVsquamous")
...
for(i in 1:length(param.names)){
	cat(i)
	tmp <- as.ts(eval(substitute(xmcmc$NAME_ARG, list(NAME_ARG=param.names[i]))))
}
@ 

<<traceplots, fig=TRUE, include=FALSE, results=hide>>=
par(ask=FALSE)
traceplots <- function(x, name)  plot(x, plot.type="single", col=1:ncol(x), ylab=name)
mapply(traceplots, params, name=names(params))
@ 
